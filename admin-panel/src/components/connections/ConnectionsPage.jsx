...
function dbTypeLabel(db_type) {
  if (db_type === 'sqlserver')  return 'SQL Server'
  if (db_type === 'mysql')      return 'MySQL'
...
export default function ConnectionsPage() {
  ...
  function connectSSE() {
    ...
    es.addEventListener('connected', e => {
      ...
      setConnections(p => ({ ...p, [b.branchId]: { ...p[b.branchId], ...b } }))
      setLastUpdate(new Date())
    })
    es.addEventListener('disconnected', e => {
      const { branchId } = JSON.parse(e.data)
      setConnections(p => { const u = { ...p }; delete u[branchId]; return u })
      setLastUpdate(new Date())
    })
    es.onerror = () => {
  ...
  async function forceSync(branchId) {
    await fetch(`${SERVER_URL}/api/branches/${branchId}/sync`, {
      method: 'POST',
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Conexiones en tiempo real</h1>
          <p className="subtitle">{online.length} conectadas · {offline.length} desconectadas</p>
        </div>
            ...
            {offline.map(b => (
              <div key={b.branch_id} className="conn-card conn-card--offline">
                <div className="conn-card-header">
                  <div>
                    <div className="conn-card-name">{b.name}</div>
                    <code style={{ fontSize:11, color:'var(--text-3)' }}>{b.branch_id}</code>
                  </div>
                  <span className={`badge badge-pos badge-${b.db_type}`}>
                    {dbTypeLabel(b.db_type)}
                  </span>
                </div>
                <div className="conn-offline-msg">
                  Sin conexión — el cliente no está corriendo o hay un error de red
                </div>
              </div>
            ))}
          </div>
...
// ── Componente separado para evitar useState dentro de map ────────────────────
function OnlineCard({ branch: b, conn, onForceSync, dbTypeLabel }) {
  const [syncing, setSyncing] = useState(false)

  return (
    <div className="conn-card conn-card--online">
      <div className="conn-card-header">
        <div>
          <div className="conn-card-name">{b.name}</div>
          <code style={{ fontSize:11, color:'var(--text-3)' }}>{b.branch_id}</code>
      ...
      <div className="conn-meta">
        <div className="conn-meta-row">
          <span className="conn-meta-label">Versión</span>
          <span>{conn?.version ?? '—'}</span>
        </div>
        <div className="conn-meta-row">
          <span className="conn-meta-label">Conectado</span>
          <span>{conn?.connectedAt ? timeAgo(conn.connectedAt) : '—'}</span>
        </div>
        <div className="conn-meta-row">
          <span className="conn-meta-label">Último sync</span>
          <span>{conn?.lastSyncAt ? timeAgo(conn.lastSyncAt) : 'Pendiente'}</span>
        </div>
        {conn?.liveData && Object.keys(conn.liveData).length > 0 && (
          <div className="conn-meta-row">
            <span className="conn-meta-label">Tipos sync</span>
            <span>{Object.keys(conn.liveData).length} activos</span>
          </div>
        )}
      </div>
      <button
