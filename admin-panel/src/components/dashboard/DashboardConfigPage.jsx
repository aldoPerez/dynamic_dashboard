...
export default function DashboardConfigPage() {
  ...
  useEffect(() => { loadBranches(); loadDataTypes(); loadAllWidgets() }, [])
  useEffect(() => { if (selectedBranch) loadLayout(selectedBranch) }, [selectedBranch])
  ...
  async function loadDataTypes() {
    const { data } = await supabase.from('data_types').select('id,key,label,is_live,columns_metadata').eq('active',true).order('sort_order')
    setDataTypes(data ?? [])
  }

  async function loadAllWidgets() {
    const { data } = await supabase.from('dashboard_widgets')
      .select('*, data_types(key,label,is_live,columns_metadata)').eq('active',true).order('created_at')
    setAllWidgets(data ?? [])
  }

  async function loadLayout(branchId) {
    setLoading(true)
    const { data } = await supabase.from('branch_dashboard_layouts')
      .select('*, dashboard_widgets(*, data_types(key,label,is_live,columns_metadata))')
      .eq('branch_id', branchId).eq('visible',true).order('sort_order')
    setLayouts(data ?? [])
    setLoading(false)
  }
  ...
  async function onDrop(e, dropIndex) {
    ...
    const updates = reordered.map((l, i) =>
      supabase.from('branch_dashboard_layouts').update({ sort_order: i }).eq('id', l.id)
    )
  ...
  async function addWidgetToLayout(widgetId) {
    await supabase.from('branch_dashboard_layouts').upsert({
      branch_id: selectedBranch, widget_id: widgetId,
      sort_order: layouts.length, visible: true,
  ...
  async function saveWidget(formData) {
    ...
    try {
      ...
      await loadAllWidgets(); await loadLayout(selectedBranch)
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div><h1>Dashboard</h1><p className="subtitle">Configura widgets y orden por sucursal</p></div>
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          ...
          {canWrite && (
            <>
              <button className="btn-secondary" onClick={() => { setEditWidget(null); setModal('add') }}>+ Agregar existente</button>
              <button className="btn-primary"   onClick={() => { setEditWidget(null); setModal('widget') }}>+ Nuevo widget</button>
            </>
          )}
        </div>
      </div>
      ...
      {loading ? <div className="loading">Cargando...</div> : layouts.length === 0 ? (
        <div className="empty">
          <span className="empty-icon">📊</span><p>Sin widgets en este dashboard</p>
          {canWrite && <button className="btn-primary" onClick={() => setModal('widget')}>Crear primer widget</button>}
        </div>
      ) : (
        <>
          <p className="drag-hint">⠿ Arrastra para reordenar — el orden se guarda automáticamente</p>
          <div className="dash-grid">
            {layouts.map((layout, i) => {
              ...
              return (
                <div key={layout.id}
                  className={`dash-widget-card ${colClass}`}
                  draggable={canWrite}
                ...
                >
                  <div className="dash-widget-header">
                    <div className="dash-widget-title">
                      <span className="dash-widget-icon">{w.kpi_icon || CHART_TYPES.find(c=>c.value===w.chart_type)?.icon || '📊'}</span>
                      <strong>{w.title}</strong>
                    </div>
                    <div className="dash-widget-meta">
                      <span className="badge" style={{ background:'var(--surface-2)',color:'var(--text-2)',fontSize:10 }}>{CHART_TYPES.find(c=>c.value===w.chart_type)?.label}</span>
                      <span className="badge" style={{ background:'var(--surface-2)',color:'var(--text-3)',fontSize:10 }}>{w.width}</span>
                      {canWrite && (
                  ...
                  <div className="dash-widget-body">
                    <div className="widget-preview">
                      ...
                      <div className="widget-preview-fields">
                        {w.chart_type==='kpi' && w.kpi_field && <span className="preview-field preview-field--kpi">Campo: {w.kpi_field}</span>}
                        ...
                        {w.chart_type==='table' && (w.y_fields??[]).map(f=><span key={f} className="preview-field preview-field--y">{f}</span>)}
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {modal==='widget' && <WidgetModal widget={editWidget} dataTypes={dataTypes} onClose={() => { setModal(null); setEditWidget(null) }} onSave={saveWidget} saving={saving} />}
      {modal==='add'    && <AddWidgetModal widgets={allWidgets} existingIds={new Set(layouts.map(l=>l.widget_id))} onClose={() => setModal(null)} onAdd={addWidgetToLayout} />}
    </div>
  )
}
...
function WidgetModal({ widget, dataTypes, onClose, onSave, saving }) {
  ...
  function changeChartType(type) { setForm(f => ({ ...f, chart_type:type, x_field:'', y_fields:[], kpi_field:'' })) }
  function changeDT(id) { setForm(f => ({ ...f, data_type_id:id, x_field:'', y_fields:[], kpi_field:'' })) }
  function toggleY(key) {
    setForm(f => ({ ...f, y_fields: f.y_fields.includes(key) ? f.y_fields.filter(k=>k!==key) : [...f.y_fields, key] }))
  }

  function validate() {
    if (!form.title.trim())   return 'El título es requerido'
    if (!form.data_type_id)   return 'Selecciona un tipo de dato'
    if (allCols.length === 0) return 'El tipo de dato no tiene columnas. Ve a Tipos de dato → Inferir desde SQL.'
    if (needsKpi && !form.kpi_field)          return 'Selecciona el campo a mostrar'
    if (needsXY  && !form.x_field)            return 'Selecciona el campo X'
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal modal-widget" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="field"><label>Tipo de gráfica *</label>
            <div className="chart-type-grid">
              {CHART_TYPES.map(ct => (
                <button key={ct.value} type="button"
                  className={`chart-type-btn ${form.chart_type===ct.value?'chart-type-btn--active':''}`}
                  onClick={() => changeChartType(ct.value)} title={ct.desc}>
          ...
          <div className="field"><label>Tipo de dato (fuente) *</label>
            <select value={form.data_type_id} onChange={e=>changeDT(e.target.value)}>
              <option value="">— Seleccionar —</option>
              {dataTypes.map(dt => (
                <option key={dt.id} value={dt.id}>[{dt.key}] {dt.label}{dt.is_live?' ● live':''}</option>
              ))}
            </select>
            {form.data_type_id && allCols.length===0 && (
              <span className="hint" style={{ color:'var(--yellow)' }}>⚠️ Sin columnas. Ve a Tipos de dato → editar → "Inferir desde SQL".</span>
            )}
          </div>
          ...
          {allCols.length > 0 && (
            <>
              ...
              {needsKpi && (
                <>
                  <div className="field"><label>Ícono del KPI</label>
                    <div className="kpi-icon-grid">
                      <button type="button"
                        className={`kpi-icon-btn ${!form.kpi_icon?'kpi-icon-btn--active':''}`}
                        onClick={() => set('kpi_icon','')}
                        title="Sin ícono">
                        <span style={{ fontSize:12, color:'var(--text-3)' }}>Sin</span>
                      </button>
                      {KPI_ICONS.map(ic => (
                        <button key={ic.emoji} type="button"
                          className={`kpi-icon-btn ${form.kpi_icon===ic.emoji?'kpi-icon-btn--active':''}`}
                          onClick={() => set('kpi_icon', ic.emoji)}
                          title={ic.label}>
                          {ic.emoji}
                        </button>
                      ))}
                  ...
                  <div className="field"><label>Campo a mostrar *</label>
                    <select value={form.kpi_field} onChange={e=>set('kpi_field',e.target.value)}>
                      <option value="">— Seleccionar columna —</option>
                      {numberCols.map(c => <option key={c.key} value={c.key}>{c.label} ({c.key})</option>)}
                    </select>
                  </div>
                  <div className="field-grid-3">
                    <div className="field"><label>Prefijo</label>
                      <input value={form.kpi_prefix} onChange={e=>set('kpi_prefix',e.target.value)} placeholder="$" />
                    </div>
                    <div className="field"><label>Sufijo</label>
              ...
              {/* Eje X */}
              {(needsXY||needsPie) && (
                <div className="field"><label>{needsPie?'Campo de etiqueta (X) *':'Eje X *'}</label>
                  <div className="cols-checkbox-group">
                    {allCols.map(c => (
                      <label key={c.key} className="checkbox-label" style={{ cursor:'pointer' }} onClick={()=>set('x_field',c.key)}>
                        <input type="radio" name="x_field" checked={form.x_field===c.key} readOnly />
                        <span>{c.label}</span><code className="key-badge">{c.key}</code>
              ...
              {(needsXY||needsPie||needsTbl) && (
                <div className="field"><label>{needsTbl?'Columnas a mostrar *':needsPie?'Campo de valor (Y) *':'Eje Y *'}</label>
                  <div className="cols-checkbox-group">
                    {(needsPie?numberCols:needsTbl?allCols:numberCols).map(c => (
                      <label key={c.key} className="checkbox-label">
                        <input type="checkbox" checked={form.y_fields.includes(c.key)} onChange={()=>toggleY(c.key)} />
                        <span>{c.label}</span><code className="key-badge">{c.key}</code>
                        <span className={`dt-col-type dt-col-type--${c.type}`}>{c.type}</span>
                      </label>
                    ))}
          ...
          {!needsKpi && !needsTbl && (
            <div className="field"><label>Colores</label>
          ...
          <div className="field"><label>Ancho en el dashboard</label>
        ...
        <div className="modal-actions">
          ...
          <button className="btn-primary" onClick={handleSave} disabled={saving}>{saving?'Guardando...':isEdit?'Guardar':'Crear widget'}</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        <div className="modal-header"><h3>Agregar widget existente</h3><button className="btn-icon" onClick={onClose}>✕</button></div>
        <div className="modal-body">
          {widgets.length===0
                ...
                {widgets.map(w => {
                  ...
                  return (
                    <div key={w.id} className={`add-widget-row ${already?'add-widget-row--added':''}`}>
                      ...
                      <button className={already?'btn-secondary':'btn-primary'}
                        style={{ fontSize:12,padding:'5px 14px',flexShrink:0 }}
                        disabled={already} onClick={()=>!already&&onAdd(w.id)}>
        ...
        <div className="modal-actions"><button className="btn-secondary" onClick={onClose}>Cerrar</button></div>
      </div>
    </div>
  )
}
