...
function onOpen() {
  ...
  // Notificar al hilo principal que ya estamos listos
...
async function runLiveSync() {
  ...
  try {
    ...
    const data      = await connector.fetchAll(keys, { dateFrom: today, dateTo: tomorrow })
...
function restartLiveSync(s) {
  stopLiveSync(); config.syncIntervalSeconds = s; startLiveSync()
}
...
async function handleQuery(msg) {
  ...
  try {
    ...
    const keys      = dataTypes?.includes('all')
      ...
      : (dataTypes ?? [])
    const data = await connector.fetchAll(keys, {
...
// Escuchar mensajes del hilo principal (ej. shutdown)
parentPort.on('message', msg => {
  if (msg.type === 'SHUTDOWN') {
    logger.info('Shutdown solicitado desde hilo principal')
    stopLiveSync()
    if (ws) ws.close()
    process.exit(0)
  }
